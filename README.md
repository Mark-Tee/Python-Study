...
```python
#类型转换
age = input("请输入你的年龄：") # 输入年龄，返回字符串类型
age = int(age) # 将输入的年龄字符串转换为整数类型
height = input("请输入你的身高：") # 输入身高，返回字符串类型
height = float(height) # 将输入的身高字符串转换为浮点数类型
```
...
转义字符是用来表示一些特殊字符的符号，通常以反斜杠（\）开头，后面跟一个字符来表示特定的含义。
常见的转义字符包括：
| 转义字符 | 说明 |
| --- | --- |
| \n | 换行符，表示在字符串中换行 |
...
if嵌套是指在一个if语句的代码块中再使用另一个if语句，可以实现更复杂的条件判断。
...
循环嵌套是指在一个循环的代码块中再使用另一个循环，可以实现更复杂的循环结构。
基本的循环嵌套结构如下：
```python
while 条件1:
    代码块1
...
字符串的查找可以使用find()方法、index()方法和in运算符等来实现。
1. find()方法返回子字符串在字符串中的起始索引位置，如果子字符串不存在，则返回-1；
2. index()方法与find()方法类似，但如果子字符串不存在，会抛出ValueError异常；
3. in运算符用于检查子字符串是否存在于字符串中，返回True或False。
...
## <span style="color:rgb(255, 146, 18)">列表、列表推导式、列表嵌套</span>
### <span style="color:rgb(144,120,249)">列表</span>
列表是一种有序、可变的集合类型，可以包含任意类型的元素。列表使用方括号（[]）定义，元素之间用逗号分隔。
以下是一些常见的列表操作示例：
...
fruits.extend(["kiwi", "melon"]) # 在列表末尾添加多个元素"kiwi"和"melon"
print(fruits) # 输出['apple', 'grape', 'blueberry', 'cherry', 'orange', 'kiwi', 'melon']

# 删除列表元素
fruits.remove("blueberry") # 删除列表中的元素"blueberry"
print(fruits) # 输出['apple', 'grape', 'cherry', 'orange', 'kiwi', 'melon']

del fruits[0] # 删除列表中的第一个元素
print(fruits) # 输出['grape', 'cherry', 'orange', 'kiwi', 'melon']

fruits.pop(1) # 删除列表中的第二个元素,不指定索引默认删除最后一个元素
print(fruits) # 输出['grape', 'cherry', 'orange', 'kiwi']

fruits.clear() # 清空列表中的所有元素
print(fruits) # 输出[]，列表已被清空
...
print(fruits[1:3]) # 输出['cherry', 'orange']，切片从索引1到索引2（不包括3）
print(fruits[1:4]) # 输出['cherry', 'orange', 'kiwi']，切片从索引1到索引3（不包括4）

# 列表是可迭代的，可以使用for循环遍历列表中的元素
for fruit in fruits:
    print(fruit) # 输出grape、cherry、orange、kiwi
...
列表推导式是一种简洁的语法，用于创建新的列表。它可以通过一个表达式和一个可迭代对象来生成一个新的列表。
基本的列表推导式结构如下：
```python
...
列表嵌套是指在一个列表中包含另一个列表，可以实现更复杂的数据结构。
基本的列表嵌套结构如下：
```python  
...
## <span style="color:rgb(255, 146, 18)">元组</span>

元组是一种有序、不可变的集合类型，可以包含任意类型的元素。元组使用圆括号（()）定义，元素之间用逗号分隔。
以下是一些常见的元组操作示例：
...
point = (3, 4)
# 访问元组元素
print(point[0]) # 输出3，访问元组的第一个元素
print(point[1]) # 输出4，访问元组的第二个元素
# 元组是不可变的，不能修改元组中的元素
# point[0] = 5 # 会抛出TypeError异常，因为元组不支持修改元素
# 元组可以包含不同类型的元素
mixed_tuple = (1, "hello", True)
print(mixed_tuple) # 输出(1, 'hello', True)
# 元组的长度
print(len(point)) # 输出2，元组point中有2个元素
# 元组的切片
print(point[0:2]) # 输出(3, 4)，切片从索引0到索引1（不包括2）
print(point[:1]) # 输出(3,)，切片从列表开头到索引0（不包括1）
print(point[1:]) # 输出(4,)，切片从索引1到列表末尾
# 元组的连接
tuple1 = (1, 2, 3)
tuple2 = (4, 5, 6)
...
# 成员检查
print(2 in tuple1) # 输出True，2是tuple1的元素
print(4 not in tuple1) # 输出True，4不是tuple1的元素
```
#### <span style="color: rgb(255, 0, 0);" >注意</span>
1. 元组是不可变的集合类型，一旦创建就不能修改元组中的元素，如果需要修改元组，可以通过重新赋值来创建一个新的元组
```python
point = (3, 4)
point = (5, 6) # 通过重新赋值创建一个新的元组
print(point) # 输出(5, 6)
```
2. 元组可以包含不同类型的元素，可以是数字、字符串、布尔值、列表等，元组可以包含不同类型的元素
```python
mixed_tuple = (1, "hello", True, [1, 2, 3])
print(mixed_tuple) # 输出(1, 'hello', True, [1, 2, 3])
```
3. 元组的索引从0开始，负数索引表示从元组的末尾开始访问元素，-1表示最后一个元素，-2表示倒数第二个元素，以此类推
```python
point = (3, 4)
print(point[-1]) # 输出4，访问元组的最后一个元素
print(point[-2]) # 输出3，访问元组的倒数第二个元素
```
4. 在使用元组操作时，要注意索引的范围，避免出现IndexError异常
```python
point = (3, 4)
print(point[2]) # 会抛出IndexError异常，因为索引2超出元组的范围
```
5. 只有当元组中只有一个元素时，需要在元素后面添加逗号，以区分元组和普通的括号表达式
```python
single_element_tuple = (5,) # 定义一个只有一个元素的元组
```
6. 元组的切片操作返回一个新的元组，原元组保持不变
```python
point = (3, 4)
sliced_point = point[0:1] # 切片从索引0到索引0（不包括1）
print(sliced_point) # 输出(3,)，切片返回一个新的元组
print(point) # 输出(3, 4)，原元组保持不变
```
7. 元组的连接和重复操作返回一个新的元组，原元组保持不变
```python
tuple1 = (1, 2, 3)
tuple2 = (4, 5, 6)
combined_tuple = tuple1 + tuple2 # 连接操作返回一个新的元组
print(combined_tuple) # 输出(1, 2, 3, 4, 5, 6)
print(tuple1) # 输出(1, 2, 3)，原元组tuple1保持不变
repeated_tuple = tuple1 * 2 # 重复操作返回一个新的元组
print(repeated_tuple) # 输出(1, 2, 3, 1, 2, 3)
print(tuple1) # 输出(1, 2, 3)，原元组tuple1保持不变
``` 
#### <span style="color: rgb(170, 206, 62);" >元组与列表的区别</span>
1. 元组是不可变的集合类型，一旦创建就不能修改元组中的元素，而列表是可变的集合类型，可以修改列表中的元素
2. 元组使用圆括号（()）定义，而列表使用方括号（[]）定义
3. 元组的元素可以是不同类型的，而列表的元素也可以是不同类型的
4. 元组的操作相对较少，主要用于存储不可变的数据，而列表的操作较多，适用于需要频繁修改数据的场景
5. 元组的性能相对于列表来说更快，因为元组是不可变的，Python对元组进行了优化，而列表需要更多的内存和处理时间来支持可变性
```python
import time
# 创建一个包含1000000个元素的列表和元组
...
for i in range(1000000):
    _ = large_list[i]
end_time = time.time()
print(f"列表访问时间: {end_time - start_time}秒")
# 测试元组的访问速度
start_time = time.time()
for i in range(1000000):
    _ = large_tuple[i]
end_time = time.time()
print(f"元组访问时间: {end_time - start_time}秒")
```

#### <span style="color: rgb(170, 206, 62);" >元组的应用场景</span>

1. 当需要存储一组不可变的数据时，使用元组更合适，例如存储坐标点、日期等
2. 当需要频繁修改数据时，使用列表更合适，例如存储学生名单、购物清单等
3. 当需要使用元组作为字典的键时，使用元组更合适，因为字典的键必须是不可变的，而列表不能作为字典的键
```python
my_dict = {(1, 2): "point A", (3, 4): "point B"} # 使用元组作为字典的键
print(my_dict[(1, 2)]) # 输出point A，访问字典中的值
```
4. 格式化输出后面的()本质上是一个元组
```python
name = "Alice"
age = 30
print("Name: %s, Age: %d" % (name, age)) # 输出Name: Alice, Age: 30，格式化输出后面的()是一个元组
```
5. 当需要使用元组来表示函数的返回值时，使用元组更合适，因为函数的返回值通常是不可变的，而列表不适合表示函数的返回值
```python
def get_coordinates():
    return (3, 4) # 返回一个元组表示坐标点
coordinates = get_coordinates()
print(coordinates) # 输出(3, 4)，函数返回一个元组
```
6. 数据不可被修改，保护数据安全

## <span style="color:rgb(255, 146, 18)">字典</span>
字典是一种无序、可变的集合类型，以键值对的形式存储数据。字典使用花括号（{}）定义，键值对之间用逗号分隔，键和值之间用冒号分隔。
以下是一些常见的字典操作示例：
...
# 访问字典元素
print(person["name"]) # 输出Alice，访问字典中键为"name"的值
print(person.get("age")) # 输出30，使用get()方法访问字典中键为"age"的值
# 修改字典元素
person["city"] = "Los Angeles" # 修改字典中键为"city"的值
print(person) # 输出{'name': 'Alice', 'age': 30, 'city': 'Los Angeles'}
# 添加元素到字典
person["email"] = "alice@example.com" # 添加一个新的键值对
print(person) # 输出{'name': 'Alice', 'age': 30, 'city': 'Los Angeles', 'email': 'alice@example.com'}
# 删除字典元素
del person["age"] # 删除字典中键为"age"的键值对
print(person) # 输出{'name': 'Alice', 'city': 'Los Angeles', 'email': 'alice@example.com'}
person.pop("city") # 删除字典中键为"city"的键值对，并返回被删除的值
print(person) # 输出{'name': 'Alice', 'email': 'alice@example.com'}
# 字典的键必须是不可变类型，值可以是任意类型
my_dict = {1: "one", "two": 2, (3, 4): "three"} # 键可以是整数、字符串、元组等不可变类型，值可以是任意类型
```

#### <span style="color: rgb(255, 0, 0);" >注意</span>
1. 字典是无序的集合类型，键值对的顺序是不固定的，在Python 3.7及以上版本中，字典保持插入顺序，但在之前的版本中，字典的顺序是不可预测的
2. 字典的键必须是不可变类型，例如整数、字符串、元组等，不能使用列表或字典作为键，因为它们是可变类型
```python
my_dict = {[1, 2]: "value"} # 会抛出TypeError异常，因为列表不能作为字典的键
```
3. 字典的值可以是任意类型，可以是数字、字符串、布尔值、列表、字典等
```python
my_dict = {"number": 42, "text": "hello", "flag": True, "list": [1, 2, 3], "dict": {"key": "value"}} # 字典的值可以是任意类型
print(my_dict) # 输出{'number': 42, 'text': 'hello', 'flag': True, 'list': [1, 2, 3], 'dict': {'key': 'value'}}
```
4. 在访问字典元素时，如果使用键访问一个不存在的键，会抛出KeyError异常，而使用get()方法访问一个不存在的键会返回None
```python
my_dict = {"name": "Alice", "age": 30}
print(my_dict["city"]) # 会抛出KeyError异常，因为键"city"在字典中不存在
print(my_dict.get("city")) # 输出None，使用get()方法访问一个不存在的键返回None
```
5. 在修改字典元素时，如果使用一个不存在的键进行赋值，会自动添加一个新的键值对到字典中
```python
my_dict = {"name": "Alice", "age": 30}
my_dict["city"] = "New York" # 使用一个不存在的键进行赋值，会自动添加一个新的键值对到字典中
print(my_dict) # 输出{'name': 'Alice', 'age': 30, 'city': 'New York'}
```
6. 在删除字典元素时，如果使用一个不存在的键进行删除，会抛出KeyError异常，而使用pop()方法删除一个不存在的键会返回None
```python
my_dict = {"name": "Alice", "age": 30}
del my_dict["city"] # 会抛出KeyError异常，因为键"city"在字典中不存在
print(my_dict.pop("city")) # 输出None，使用pop()方法删除一个不存在的键返回None
``` 
7. 字典的键是唯一的，如果在字典中添加一个已经存在的键，会覆盖原有的值
```python
my_dict = {"name": "Alice", "age": 30}
my_dict["name"] = "Bob" # 添加一个已经存在的键，会覆盖原有的值
print(my_dict) # 输出{'name': 'Bob', 'age': 30}
```
8. 字典的键值对之间用逗号分隔，键和值之间用冒号分隔，在定义字典时要注意语法格式
```python
my_dict = {"name": "Alice", "age": 30, "city": "New York"} # 定义一个字典，键值对之间用逗号分隔，键和值之间用冒号分隔
print(my_dict) # 输出{'name': 'Alice', 'age': 30, 'city': 'New York'}
```
9. 字典的操作相对于列表和元组来说更复杂，因为字典是无序的集合类型，不能通过索引访问元素，而是通过键来访问元素
```python
my_dict = {"name": "Alice", "age": 30, "city": "New York"}
print(my_dict[0]) # 会抛出KeyError异常，因为字典不能通过索引访问元素
print(my_dict["name"]) # 输出Alice，通过键"name"访问元素
```
10. 字典的键必须是不可变类型，值可以是任意类型，但如果值是可变类型，例如列表或字典，修改这个值会影响到字典中的数据
```python
my_dict = {"name": "Alice", "hobbies": ["reading", "traveling"]}
my_dict["hobbies"].append("cooking") # 修改字典中键"hobbies"对应的值，这个值是一个列表，修改这个列表会影响到字典中的数据
print(my_dict) # 输出{'name': 'Alice', 'hobbies': ['reading', 'traveling', 'cooking']}
``` 
